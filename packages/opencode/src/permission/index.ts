...
function effective(permission: string, pattern: string, ...rulesets: PermissionV1.Ruleset[]) {
  return Compliance.restrict(evaluate(permission, pattern, ...rulesets), permission, pattern)
}

function relevant(permission: string, ...rulesets: ReadonlyArray<PermissionV1.Rule>[]) {
...
export function disabled(tools: string[], ruleset: PermissionV1.Ruleset): Set<string> {
  ...
  return new Set(
    tools.filter((tool) => {
      ...
      if (Compliance.disables(permission)) return true
