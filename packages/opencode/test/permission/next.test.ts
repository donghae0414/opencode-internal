...
it.instance(
  "ask - compliance ask overrides user allow",
  () =>
    Effect.gen(function* () {
      const fiber = yield* ask({
        sessionID: SessionID.make("session_test"),
...
it.instance(
  "ask - compliance deny overrides user allow",
  () =>
    Effect.gen(function* () {
      const err = yield* fail(
        ask({
...
it.instance(
  "ask - user deny remains stronger than compliance allow",
  () =>
    Effect.gen(function* () {
      const err = yield* fail(
        ask({
...
it.instance(
  "ask - compliance ask overrides user allow for webfetch",
  () =>
    Effect.gen(function* () {
      const fiber = yield* ask({
        sessionID: SessionID.make("session_test"),
...
it.instance(
  "ask - compliance deny overrides user allow for websearch",
  () =>
    Effect.gen(function* () {
      const err = yield* fail(
        ask({
...
it.instance(
  "ask - non-bash permissions are unchanged",
  () =>
    Effect.gen(function* () {
      const result = yield* ask({
        sessionID: SessionID.make("session_test"),
...
it.instance(
  "reply - always approval cannot exceed compliance ask",
  () =>
    Effect.gen(function* () {
      const a = yield* ask({
        id: PermissionV1.ID.make("per_git"),
        sessionID: SessionID.make("session_test"),
        permission: "bash",
        patterns: ["git status"],
        metadata: {},
      ...
      yield* waitForPending(1)
      yield* reply({ requestID: PermissionV1.ID.make("per_git"), reply: "always" })
      yield* Fiber.join(a)

      // a later ask in a new session must still be forced to ask by compliance
      const b = yield* ask({
...
it.instance(
  "reply - always approval cannot exceed compliance deny",
  () =>
    Effect.gen(function* () {
      const a = yield* ask({
        id: PermissionV1.ID.make("per_any"),
        sessionID: SessionID.make("session_test"),
        permission: "bash",
        patterns: ["git status"],
        metadata: {},
      ...
      yield* waitForPending(1)
      yield* reply({ requestID: PermissionV1.ID.make("per_any"), reply: "always" })
      yield* Fiber.join(a)
